...
robodojo-scene-expansion #29, M2) -- RoboDojo's `sort_nesting_dolls_by_size` task. Reuses
../robodojo_pusht/robodojo_pusht_env_cfg.py's exact robot/table/camera setup unchanged (same
dual-X5 rig, same mount poses), matching every other robodojo_* env cfg in this fork; a
WAN-hard scene: 5 same-texture matryoshka dolls at 5 sizes (indices 0-4, `texture0` group,
fixed for this pass -- see the Genesis-side scene module's docstring for the full
`sort_nesting_dolls_by_size.yml` rationale).
...
pipeline) -- no MeshConverter physics-baking step needed, same plain UsdFileCfg(usd_path=...)
as every other robodojo_* object.
...
(doll0=largest). Spawned with `func=spawn_usd_file_with_mass` (`..pusht.mdp.spawners`) instead
of `UsdFileCfg`'s own default `func` (see that spawner's docstring for why: RoboDojo's
`object.usdz` has no pre-authored `MassAPI`).
...
# Matches simulation/robodojo_sort_nesting_dolls_by_size/scene.py's DOLL_INDEX/
# DOLL_INIT_POS exactly -- (staged doll index dir, prim name, init pos).
_DOLL_SPECS: dict[str, tuple[str, str, tuple[float, float, float]]] = {
    "doll0": ("matryoshka_dolls_00000", "Doll0", (-0.30, -0.15, 0.0853)),
    "doll1": ("matryoshka_dolls_00001", "Doll1", (-0.15, -0.15, 0.0752)),
    "doll2": ("matryoshka_dolls_00002", "Doll2", (0.00,  -0.15, 0.0652)),
    "doll3": ("matryoshka_dolls_00003", "Doll3", (0.15,  -0.15, 0.0552)),
    "doll4": ("matryoshka_dolls_00004", "Doll4", (0.30,  -0.15, 0.0451)),
}
...
def _doll_cfg(name: str) -> RigidObjectCfg:
    ...
    staged_dir, prim_name, pos = _DOLL_SPECS[name]
...
@configclass
class RobodojoSortNestingDollsBySizeSceneCfg(InteractiveSceneCfg):
    """Tabletop bimanual Sort-Nesting-Dolls-By-Size scene using two ARX X5 arms + 5
    same-texture matryoshka dolls at 5 sizes."""
    ...
    doll0 = _doll_cfg("doll0")
    doll1 = _doll_cfg("doll1")
    doll2 = _doll_cfg("doll2")
    doll3 = _doll_cfg("doll3")
    doll4 = _doll_cfg("doll4")
...
class EventCfg:
    ...
    reset_doll0 = EventTerm(
        func=base_mdp.reset_root_state_uniform,
        mode="reset",
        params={"pose_range": {}, "velocity_range": {}, "asset_cfg": SceneEntityCfg("doll0")},
    )
    reset_doll1 = EventTerm(
        func=base_mdp.reset_root_state_uniform,
        mode="reset",
        params={"pose_range": {}, "velocity_range": {}, "asset_cfg": SceneEntityCfg("doll1")},
    )
    reset_doll2 = EventTerm(
        func=base_mdp.reset_root_state_uniform,
        mode="reset",
        params={"pose_range": {}, "velocity_range": {}, "asset_cfg": SceneEntityCfg("doll2")},
    )
    reset_doll3 = EventTerm(
        func=base_mdp.reset_root_state_uniform,
        mode="reset",
        params={"pose_range": {}, "velocity_range": {}, "asset_cfg": SceneEntityCfg("doll3")},
    )
    reset_doll4 = EventTerm(
        func=base_mdp.reset_root_state_uniform,
        mode="reset",
        params={"pose_range": {}, "velocity_range": {}, "asset_cfg": SceneEntityCfg("doll4")},
    )
...
@configclass
class RobodojoSortNestingDollsBySizeEnvCfg(ManagerBasedRLEnvCfg):
    scene: RobodojoSortNestingDollsBySizeSceneCfg = RobodojoSortNestingDollsBySizeSceneCfg(
